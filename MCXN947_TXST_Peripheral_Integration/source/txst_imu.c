...
/*
 * If successful, prints i3c WHO_AM_I register.
...
static int txst_i3c_init (void) {
    ...
    uint32_t freq = I3C_MASTER_CLOCK_FREQUENCY;
    PRINTF("I3C clock freq: %u\r\n", freq);
    ...
    //g_master_txBuff[0]      = 0x06U;

    memset(&masterXfer, 0, sizeof(masterXfer));

    masterXfer.slaveAddress   = 0x7EU; /* Broadcast address */
    //masterXfer.slaveAddress   = 0x6AU; /* Broadcast address */
    masterXfer.subaddress = 0x86U; /* CCC command RSTDAA */
    ...
    //PRINTF("RSTDAA done: %d\r\n", result);   // Does this print?
    //PRINTF("HERE");
    //PRINTF("made it here");
    if (kStatus_Success != result)
    {
    	PRINTF("FAILED TO SET DYNAMIC ADDRESS");
        return result;
    }
